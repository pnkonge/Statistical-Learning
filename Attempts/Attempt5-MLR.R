...
training_data$X12 <- as.factor(training_data$X12)
is.factor(training_data$X12)
...
sig_lm1 <- lm(y~X1+X2+X3+X4+X5+X8+X12+X13+X23+X25, data = training_data)
summary(sig_lm1)
...
sig_with_inter_lm <- lm(y~(X1+X2+X3+X4+X5+X8+X13+X23+X25)*X12, data = training_data)
summary(sig_with_inter_lm)
plot(sig_with_inter_lm$residuals)

#Final linear model
final_lm <- lm(y~X1+X23+X25+X4+X8+X12, data = training_data)
summary(final_lm)
...
names(solution)
nrow(solution)
write.csv(solution, file = "5_M.L.R_prediction_answer.csv", row.names = FALSE)
