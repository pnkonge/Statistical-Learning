...
# fit lasso models
fit.la = glmnet(x_train,y_train,alpha =1, lambda = grid)

#Cross Validation
#set.seed(1)
cv.la = cv.glmnet(x_train,y_train,alpha =1) # cross validation for lasso
...
nrow(lasso_solution)
write.csv(lasso_solution, file = "3_lasso_prediction_answer.csv", row.names = FALSE)
